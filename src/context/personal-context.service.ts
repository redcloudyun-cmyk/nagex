...
export class PersonalContextService {
  ...
  public getRelevantMemories(
    ...
    // Exclude DELETED / EXPIRED / SUPERSEDED / CONFLICTED / PROPOSED (getActiveMemories already filters lifecycle === 'ACTIVE')
    const validActive = activeMemories.filter(
      (m) => m.lifecycle === 'ACTIVE' && m.tenantId === tenantId && m.owner_id === principalId && m.sensitivity !== 'S3'
    );
    ...
    const scored = validActive.map((mem) => {
      ...
      const val = typeof mem.content?.value === 'string'
      ...
      if (mem.pinned && (matchCount > 0 || queryTokens.length === 0)) {
        score += 2;
      } else if (mem.pinned && matchCount === 0 && queryTokens.length > 0) {
