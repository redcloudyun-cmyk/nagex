...
// runs. Without this, PersistentActionApprovalStore/ExecutionStore would
// default to /var/lib/nagex (or ~/.local/share/nagex on a dev machine) and
// litter the real filesystem with test data on every `npm test` run. The
// Google OAuth token store is safe by construction (it only ever writes when
// NAGEX_TOKEN_ENCRYPTION_KEY is set, which tests never set), but it's
// redirected too for defense in depth.
