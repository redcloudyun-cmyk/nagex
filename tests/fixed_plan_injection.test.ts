// V01a — Test-Only Deterministic Plan Injection.
//
...
// route does not exist (falls through to the ordinary 404) unless
// NAGEX_ENABLE_TEST_PLAN_INJECTION is exactly '1', re-checked per request;
// (3) when active, it drives a real Task run through the real
// step-execution/durable-state/approval-continuation path for a fixed,
// deterministic 3-step plan (read-only -> write/APPROVAL_REQUIRED ->
// read-only), exactly like V01's LIVE harness needs.
...
// ── 2-4. The route does not exist unless the env var is exactly '1' ──────
...
test('2. the route 404s (indistinguishable from a nonexistent route) when the env var is unset', async () => {
  delete process.env.NAGEX_ENABLE_TEST_PLAN_INJECTION;
  const taskId = await createRealTask();
  const withoutFlag = await handleAsyncApiRequest('POST', `/api/v1/tasks/${taskId}/run-with-fixed-plan`, { steps: [rawStep({})] }, HEADERS);
...
test('3. the route 404s when the env var is an empty string or any value other than exactly "1"', async () => {
  const taskId = await createRealTask();
  for (const value of ['', 'true', '0', 'TRUE', '1 ']) {
    ...
    assert.equal(res.status, 404, `value ${JSON.stringify(value)} must not enable the route`);
  }
  delete process.env.NAGEX_ENABLE_TEST_PLAN_INJECTION;
});

test('4. the route returns 400 (not a crash) for a missing/empty steps array, even with the env var on', async () => {
  process.env.NAGEX_ENABLE_TEST_PLAN_INJECTION = '1';
  try {
    const taskId = await createRealTask();
    const missing = await handleAsyncApiRequest('POST', `/api/v1/tasks/${taskId}/run-with-fixed-plan`, {}, HEADERS);
    assert.equal(missing.status, 400);
    const empty = await handleAsyncApiRequest('POST', `/api/v1/tasks/${taskId}/run-with-fixed-plan`, { steps: [] }, HEADERS);
...
// ── 5. When active, the route genuinely reaches the real production ──────
// pipeline (PlanResolver -> ExecutingTaskRunner -> real CapabilityBroker),
// not a stub. This local/CI environment has no live Google OAuth
// configured (confirmed: the real ToolRegistry reports gmail.search
// UNAVAILABLE here, exactly like every other test file in this suite that
// exercises live Gmail/Calendar through the real app graph), so the real,
// honest, deterministic-for-this-environment outcome is STEP_BLOCKED at
// resolution — not a fabricated success. The full pause -> approve ->
// resume lifecycle this route unlocks is proven separately: in-process
// with a controlled fake broker (test 6 below, exactly like
...
test('5. active route reaches the real PlanResolver/ExecutingTaskRunner/CapabilityBroker pipeline and produces a truthful, persisted TaskRun', async () => {
  process.env.NAGEX_ENABLE_TEST_PLAN_INJECTION = '1';
  try {
    const taskId = await createRealTask();
    const steps = [rawStep({ step: 1, title: 'Search first', tool: 'gmail.search', dependsOn: [], parameters: { query: 'invoice' } })];

    const runRes = await handleAsyncApiRequest('POST', `/api/v1/tasks/${taskId}/run-with-fixed-plan`, { steps }, HEADERS);
...
// ── 6. runWithResolvedPlan() supports the full pause -> approve -> resume ─
...
// through this new entry point specifically).
test('6. via runWithResolvedPlan(): step 1 executes, step 2 pauses at a real approval, step 3 does not run until approved — then the whole run completes', async () => {
