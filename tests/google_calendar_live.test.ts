...
test('token refresh: an expired access token is transparently refreshed using the stored refresh token', async () => {
  ...
  const accessToken = await tokenStore.getValidAccessTokenForPrincipal('t1', 'usr_1', config, fetchFn, 'req_refresh_1', () => clock);
...
test('token refresh failure clears the connection (fail closed) instead of returning a stale token', async () => {
  ...
  const accessToken = await tokenStore.getValidAccessTokenForPrincipal('t1', 'usr_1', config, fetchFn, 'req_refresh_2');

  assert.equal(accessToken, null);
  assert.equal(tokenStore.isConnectedForPrincipal('t1', 'usr_1'), false);
});
...
test('execution success: returns a normalized SUCCEEDED result and never fakes success without calling Google', async () => {
  ...
  tokenStore.saveForPrincipal('t1', 'u1', { accessToken: 'at', refreshToken: 'rt', expiresAt: Date.now() + 3600_000, scope: GRANTED_SCOPE_STRING });
...
test('audit success/failure: every stage of the approval + execution lifecycle is logged with the dotted action names, tokens are never logged', async () => {
  ...
  tokenStore.saveForPrincipal('t1', 'u1', { accessToken: 'super-secret-access-token', refreshToken: 'super-secret-refresh-token', expiresAt: Date.now() + 3600_000, scope: GRANTED_SCOPE_STRING });
...
test('memory update after success: writes "Scheduled <summary> for <date/time>." without persisting attendee emails', async () => {
  ...
  tokenStore.saveForPrincipal('t1', 'usr_mem_test', { accessToken: 'at', refreshToken: 'rt', expiresAt: Date.now() + 3600_000, scope: GRANTED_SCOPE_STRING });
